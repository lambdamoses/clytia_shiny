...
```{r}
cluster_colors <- tibble(res.0.8 = as.character(0:15),
                         color = hue_pal()(16))
clytia@meta.data <- clytia@meta.data %>% 
  left_join(cluster_colors, by = "res.0.8")
colors_use <- setNames(ac(clytia@meta.data$color, alpha = 0.5),
                       colnames(clytia@scale.data))
```

```{r}
show1 <- show.velocity.on.embedding.cor(emb = clytia@dr$tsne@cell.embeddings, vel = clytia_vel, show.grid.flow = TRUE, arrow.scale = 3, grid.n = 40, cell.colors = colors_use, cc = show1$cc)
```

```{r}
show.velocity.on.embedding.cor(emb = clytia@dr$pca@cell.embeddings[,1:2], vel = clytia_vel, cell.colors = colors_use, show.grid.flow = TRUE, arrow.scale = 2, grid.n = 40, cc = show1$cc)
```
...
clytia_cell_attrs <- tibble(cell_names = clytia_loom[["col_attrs/cell_names"]][], 
                            nGene = clytia_loom[["col_attrs/nGene"]][],
