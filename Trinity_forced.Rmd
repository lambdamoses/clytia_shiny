...
```{r}
rownames(spliced) <- rownames(unspliced) <- rownames(ambiguous) <- 
...
Are there doublets?
```{r}
clytia <- doubletFinder(clytia, expected.doublets = 100)
```
```{r}
hist(clytia@meta.data$pANN, breaks = 100)
```

This histogram looks quite different from what I've seen in mouse brain datasets. It doesn't have a prominent peak with pANN greater than about 0.6. 
```{r}
clytia@meta.data %>% 
...
```{r}
TSNEPlot(clytia, group.by = "pANNPredictions", pt.size = 0.5)
```
...
It was said that the first PC corresponds to sequencing depth. Is this really the case?
```{r}
FeaturePlot(clytia, "nUMI", reduction.use = "pca", cols.use = viridis(256))
```

No, it doesn't seem so, so I'll continue to use PC1.

Burning question: How many PCs to use? It was said that PCA is denoising; using too many PCs will include more noise, while using too few will lose too much signal. A way built into Seurat to determine the number of PCs is JackStraw, which does permutation to estimate a null distribution to estimate p-values of PCs.

```{r}
clytia <- JackStraw(clytia, do.par = TRUE, num.pc = 75)
```

```{r, fig.width = 6, fig.height = 10}
JackStrawPlot(clytia, PCs = 1:40, nCol = 5)
```

OK, if I just use the p-values, then I'd better stop at around 30 PCs. 
```{r}
# Clustering
clytia <- clytia %>% 
  FindClusters(genes.use = clytia@var.genes, dims.use = 1:34, resolution = 1)
```
...
It doesn't look too different from when I used 60 PCs.
```{r}
clytia <- clytia %>% 
  RunTSNE(dims.use = 1:34)
clytia <- SetAllIdent(clytia, "res.1")
TSNEPlot(clytia, pt.size = 0.5)
```

```{r}
clytia_umap <- umap(clytia@dr$pca@cell.embeddings[,1:34], n_neighbors = 50, n_components = 2)
```
...
gene_accession %>% 
  filter(gene == "XLOC_030664")
```

```{r}
FeaturePlot(clytia, "XLOC_051264", cols.use = viridis(256), pt.size = 0.5, reduction.use = "tsne")
```
...
```{r}
cluster_colors <- tibble(res.1 = as.character(0:26),
                         color = hue_pal()(27))
...
```{r}
show1 <- show.velocity.on.embedding.cor(emb = clytia@dr$tsne@cell.embeddings, vel = clytia_vel, show.grid.flow = TRUE, arrow.scale = 5, grid.n = 40, cell.colors = colors_use, cex = 0.5, cc = show1$cc)
```
